...
public class DigestAdminParticipantWidget extends Composite implements RunnableOnTabSelect {
	...
	interface DigestAdminWidgetUiBinder extends
			UiBinder<Widget, DigestAdminParticipantWidget> {
	}
	...
	AsyncCallback<JSONValue> digestAdminParticipantCallback =  new AsyncCallback<JSONValue>() {
	...
	@Inject
	public DigestAdminParticipantWidget(final DigestMessages messages, final DigestConstants constants, final GlobalResources resources, final DigestService digestService, final VegaUtils vegaUtils) {
	...
	public void initRemoveHandlers(final DigestMessages messages,
			final DigestConstants constants, final DigestService digestService,
			final VegaUtils vegaUtils) {
...
//		clearAll();
		if(isFirstInit){
			initRemoveHandlers(messages, constants, digestService, vegaUtils);
			isFirstInit = false;
		}
	...
	public AsyncCallback<JSONValue> getDigestAdminParticipantCallback() {
		return digestAdminParticipantCallback;
	}
